...
export default class BuildCreneaux {
  ...
  async dateBorne () {
    ...
    if (this.date_max.isValid()) {
      let h_debut = this.h_debut
      let m_debut = this.m_debut
      let h_fin = this.h_fin
      let m_fin = this.m_fin
      const day_indice = this.date_max.day()
      ...
      if (this.plage_heures_valide.length > 0) {
        if (this.plage_heures_valide[day_indice]) {
          const h_d = this.plage_heures_valide[day_indice].debut.split(':')
          h_debut = parseInt(h_d[0])
          m_debut = parseInt(h_d[1])
          const h_f = this.plage_heures_valide[day_indice].fin.split(':')
  ...
  getIntervalUtilisable (date_min_string) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      const bloc_date = {}
      const date_time = moment(date_min_string, 'DD-MM-YYYY HH:mm:ss')
      const date_time_end = moment(date_min_string, 'DD-MM-YYYY HH:mm:ss').add(
        self.app_interval,
        'minutes'
      )
      const date_time_next_creneau = moment(
        date_min_string,
        'DD-MM-YYYY HH:mm:ss'
