    ...
    <transition name="fade" mode="out-in">
      <div class="creneaux-mbt--block mt-5" v-show="show">
        ...
        <type-livraison
          ...
          :price_shipping="newpriceshipping"
          ref="typelivraison"
        >
...
var ShopifyformatMoney = function(cents, format) {
  var value = "",
  ...
  methods: {
    ...
    async getProductType() {
      if (this.cart && this.cart.item_count > 0) {
        console.log("getProductType()", this.typeLivraisonIds);
    ...
    async CartTypeLivraisonUpdate() {
      if (this.cart && this.cart.item_count > 0) {
        if (this.CartTypeLivraison && this.CartTypeLivraison !== "") {
          ...
          await this.addProductTypeLivraison();
        }
      } else {
    ...
    newpriceshipping: {
      get() {
        if (this.cart.total_price) {
          ...
          $(".panier .sup-total .money").html(
            ShopifyformatMoney(
              this.cart.items_subtotal_price,
              "{{amount_with_comma_separator}} €"
            )
          );
          $(".panier .total .money").html(
            ShopifyformatMoney(
        ...
        if (
          ...
          $(".panier .price-livraion").html("offert");
          return "offert";
        }
      }
    }
